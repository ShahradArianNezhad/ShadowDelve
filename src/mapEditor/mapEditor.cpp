...
void MapEditor::closeSelectMenu(){
...
void MapEditor::loadTileMap(){
  ...
  for(auto& [tileX,tileYtoName]:data["entity"].items()){
    for(auto& [tileY,name]:tileYtoName.items()){
      ...
      entityMap[gridX][gridY]=EditorEntity{id,name};
    }
  }
...
void MapEditor::nextTile(EntityId entity){
  if(uvMax.x<1){
    uvMax.x+=1.0/uvSizeX;
    uvMin.x+=1.0/uvSizeX;
  }else{
...
void MapEditor::prevTile(EntityId entity){
  if(uvMin.x>0){
    uvMax.x-=1.0/uvSizeX;
    uvMin.x-=1.0/uvSizeX;
  }else{
...
void MapEditor::placeAtMousePos(){
  if(mode=="select"){
    return;
  };
  ...
  else entityMap[gridX][gridY]=EditorEntity{id,mode};
}

void MapEditor::deleteAtMousePos(){
...
void MapEditor::showPreviewAtMousePos(vec2 pos){
